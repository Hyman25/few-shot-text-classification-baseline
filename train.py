...
def train_and_validate(args):
    # 1. load data
    fold_time = time.strftime("%Y%m%d_%H%M", time.localtime())
    if not os.path.exists(f'{args.savedmodel_path}/fold_{fold_time}'): os.makedirs(f'{args.savedmodel_path}/fold_{fold_time}')
    ...
    for epoch in range(args.max_epochs):
            ...
            if i % (100000//args.batch_size//4) == 0 and i > 0 and i < (100000//args.batch_size-100000//args.batch_size//3-100) and epoch>1:
                ...
                if mean_f1 >= best_score:
                    ...
                    torch.save({'epoch': epoch, 'model_state_dict': model.module.state_dict(), 'mean_f1': mean_f1},
                                f'{args.savedmodel_path}/fold_/model_epoch_{epoch}_{i}_mean_f1_{mean_f1}.bin')
                    best_score = mean_f1
        ...
        if mean_f1 > best_score:
            best_score = mean_f1
            torch.save({'epoch': epoch, 'model_state_dict': model.module.state_dict(), 'mean_f1': mean_f1},
                    f'{args.savedmodel_path}/fold_{fold_time}/model_epoch_{epoch}_mean_f1_{mean_f1}.bin')
        if args.ema:
            #--------ema restore-------
            ema.restore()
            #--------------------------
